...
from bag.layout.routing.fill import fill_symmetric_min_density_info, fill_symmetric_interval, fill_symmetric_max_density
...
class MOSTechPlanarGeneric(MOSTech):
    ...
    def get_od_w(self, lch_unit, fg):
        # type: (int, int) -> int
        """Calculate OD width from number of fingers"""
        mos_constants = self.get_mos_tech_constants(lch_unit)
        sd_pitch = mos_constants['sd_pitch']
        po_od_extx = mos_constants['po_od_extx']
        od_fill_w = mos_constants['od_fill_w'][0]
        return max(od_fill_w, (fg - 1) * sd_pitch + lch_unit + 2 * po_od_extx)

    def get_od_w_inverse(self, lch_unit, w, round_up=None):
        # type: (int, int, bool) -> int
        """Calculate number of fingers from OD width."""
        mos_constants = self.get_mos_tech_constants(lch_unit)
        sd_pitch = mos_constants['sd_pitch']
        po_od_extx = mos_constants['po_od_extx']
    ...
    def get_od_h(self, lch_unit, w):
        # type: (int, float) -> int
        """Calculate OD height from transistor width"""
        mos_constants = self.get_mos_tech_constants(lch_unit)
        po_od_exty = mos_constants['po_od_exty']
        return w + 2 * po_od_exty

    def get_od_h_inverse(self, lch_unit, od_h):
        # type: (int, int) -> int
        """Calculate transistor width from OD height."""
        mos_constants = self.get_mos_tech_constants(lch_unit)
        po_od_exty = mos_constants['po_od_exty']
        return od_h - 2 * po_od_exty

    def _get_dummy_od_yloc(self,  # type: MOSTechPlanarGeneric
                           lch_unit,  # type: int
                           yblk,  # type: int
        ...
        When bot_od_h, bot_od_yt, top_od_h, top_od_yb are provided, we are filling space
        between two rows. Otherwise, we are filling empty space.
        """
        guard_ring_nf = kwargs.get('guard_ring_nf', 0)

        mos_constants = self.get_mos_tech_constants(lch_unit)
        od_min_density = kwargs.get('od_min_density', mos_constants['od_min_density'])
        od_spy = mos_constants.get('dod_spy', mos_constants['od_spy'])
        od_spy_max = mos_constants.get('od_spy_max', mos_constants['od_spy_max'])
        od_w_min, od_w_max = mos_constants['od_fill_w']
        po_spy = mos_constants['po_spy']
        po_od_exty = mos_constants['po_od_exty']
        dpo_edge_spy = mos_constants['dpo_edge_spy']

        if not(guard_ring_nf == 0 or 'od_spy_max_gr' not in mos_constants):
        ...
        if top_od_yb is not None:
            ...
            offset = bot_od_yt
            od_fin_area_min = self.get_od_h_inverse(lch_unit, od_area_targ)
            od_fin_area_tot = top_od_yb - bot_od_yt + 1
            foe = False
        else:
            ...
            dum_h_min = self.get_od_h(lch_unit, od_w_min)
            ...
            if fill_h < dum_h_min * 2 + po_spy:
                ...
                od_w = min(od_w_max, self.get_od_h_inverse(lch_unit, fill_h))
                od_h = self.get_od_h(lch_unit, od_w)
                od_yb = (fill_yb + fill_yt - od_h) // 2
                return [(od_yb, od_yb + od_h)]

            # compute OD fill area needed to meet density
            bot_od = fill_yb + po_od_exty
            top_od = fill_yt - po_od_exty

            od_area_tot = yblk
            od_area_targ = int(math.ceil(od_area_tot * od_min_density))
            od_fin_area_min = self.get_od_h_inverse(lch_unit, od_area_targ)
            od_fin_area_tot = top_od - bot_od
    ...
    def get_valid_extension_widths(self, lch_unit, top_ext_info, bot_ext_info, **kwargs):
        ...
        od_sp_margin = top_ext_info.margins['od'][0] + bot_ext_info.margins['od'][0]
    ...
    def _get_dummy_po_y_list(self, lch_unit, bot_ext_info, top_ext_info, yblk):
        ...
        while po_area_iter.has_next():
            ...
            fill_info = fill_symmetric_min_density_info(
                po_area_tot,
                po_area_targ,
                n_min_po,
                n_max_po,
        ...
        return fill_symmetric_interval(
    ...
    def get_ext_info(self, lch_unit, w, fg, top_ext_info, bot_ext_info, **kwargs):
        ...
        for od_y, po_y in zip(od_y_list, po_y_list):
            ...
            row_info_list.append(
                RowInfo(od_x=(0, fg), od_y=od_y, od_type=('dum', cur_sub_type), po_y=po_y))
    ...
    def get_sub_ring_ext_info(self, sub_type, height, fg, end_ext_info, **kwargs):
        ...
        for od_y, po_y in zip(od_y_list, po_y_list):
            row_info_list.append(
                RowInfo(od_x=(0, 0), od_y=od_y, od_type=('dum', sub_type), po_y=po_y))
        ...
        layout_info = dict(
            ...
            sub_type=sub_type,
            imp_params=[
                (sub_type, end_ext_info.thres, 0, height, 0, height),
            ],
            is_sub_ring=True,
            dnw_mode=dnw_mode,
        )
    ...
    def get_substrate_info(self, lch_unit, w, sub_type, threshold, fg, blk_pitch=1, **kwargs):
        ...
        blk_yb, blk_yt = blk_y = yloc_info['blk']
        od_yb, od_yt = od_y = yloc_info['od']
        d_mx_yb, d_mx_yt = d_mx_y = yloc_info['d_conn_y']
        od_h = od_yt - od_yb
        od_yc = (od_yb + od_yt) // 2
    ...
    def _get_end_blk_info(self, lch_unit, sub_type, threshold, fg, is_end, blk_pitch, **kwargs):
        ...
        if is_sub_ring_end:
            ...
            imp_params = [
                (sub_type, threshold, 0, arr_yt, 0, arr_yt),
            ]
            ext_info = ExtInfo(
                margins=dict(
    ...
    def get_outer_edge_info(self, guard_ring_nf, layout_info, is_end, adj_blk_info, **kwargs):
        ...
        if blk_type != 'sub' and guard_ring_nf == 0:
            ...
            for row_info in row_info_list:
                new_row_list.append(
                    RowInfo(od_x=(0, 0), od_y=(0, 0), od_type=row_info.od_type, po_y=row_info.po_y))
    ...
    def get_gr_sub_info(self, guard_ring_nf, layout_info, **kwargs):
        ...
        row_info_list = [RowInfo(od_x=od_x_main, od_y=od_y, od_type=('sub', sub_type), po_y=(0, 0))]
        if blk_type == 'sub':
            row_info_list.append(
                RowInfo(
                    od_x=(fg_od_margin, fg_od_margin + guard_ring_nf),
                    od_y=(od_y[1], arr_y[1]),
                    od_type=('sub', sub_type),
                    po_y=(0, 0)))
    ...
    def get_gr_sep_info(self, layout_info, adj_blk_info, **kwargs):
        ...
        mos_constants = self.get_mos_tech_constants(lch_unit)
        fg_gr_min = mos_constants['fg_gr_min']
        edge_constants = self.get_edge_info(
            lch_unit, fg_gr_min, True, is_sub_ring=is_sub_ring, dnw_mode=dnw_mode)
        ...
        if blk_type == 'sub':
            ...
            new_row_list.append(
                RowInfo(od_x=(0, fg_gr_sep), od_y=sub_y_list[0], od_type=od_type, po_y=(0, 0)))
        else:
            cur_blk_type = 'gr_sep'
            sub_y_list = None
            if is_sub_ring:
                ...
                for row_info in row_info_list:
                    new_row_list.append(
                        RowInfo(od_x=(0, 0), od_y=(0, 0), od_type=od_type, po_y=row_info.po_y))
    ...
    def draw_mos(self, template, layout_info):
        ...
        for row_info in row_info_list:
            ...
            if od_yt > od_yb and draw_od:
                ...
                self.draw_od(
                    template,
                    od_lay_cur,
                    BBox(od_xl, od_yb, od_xr, od_yt, res, unit_mode=True),
                    od_type='main')
            ...
            if po_yt > po_yb:
                for idx in range(fg):
                    ...
                    lay = po_lay if (cur_od_type == 'mos' or cur_od_type == 'mos_fake'
                                     or cur_od_type == 'sub') else po_dum_lay
        ...
        if (blk_type == 'sub' or blk_type == 'gr_sub' or blk_type == 'gr_sub_sub'
                or blk_type == 'gr_sep_sub'):
            sub_m1_extx = mos_constants['sub_m1_extx']
    ...
    def draw_substrate_connection(
            self,  # type: MOSTechPlanarGeneric
        ...
        if exc_tracks:
            # TODO: fix this
            print('WARNING: exc_tracks options not current supported in this technology')
        ...
        if mx_yt > mx_yb:
            ...
            via_drawn = self._draw_vertical_vias(
                template,
                lch_unit,
                x0,
                num_via,
    ...
    def draw_mos_connection(
        ...
        mos_conn_layer = self.get_mos_conn_layer()
        s_warr = template.add_wires(
            mos_conn_layer, -0.5, mx_yb, mx_yt, num=num_s, pitch=stack * 2, unit_mode=True)
        d_warr = template.add_wires(
            mos_conn_layer, stack - 0.5, mx_yb, mx_yt, num=num_d, pitch=stack * 2, unit_mode=True)

        if diode_conn:
            d_warr = WireArray.list_to_warr(template.connect_wires([
                d_warr,
            ] + g_warrs))
    ...
    def draw_active_fill(self, template, mos_type, threshold, w, h):
        ...
        """Draw active fill in given area

        w and h are expected in unit mode
        """
        mos_layer_table = self.config['mos_layer_table']
        ...
        po_min_density = mos_constants.get('po_min_density', None)
        if po_min_density:
            po_x_area = 0
            for od_xl, od_xr in od_x_list:
                ...
                po_x_area += nx * lch_unit
            po_x_density = po_x_area / w
            po_y_density = po_min_density / po_x_density
            density_targ = max(od_y_density, po_y_density)
        else:
            density_targ = od_y_density

        od_y_list = self._get_dummy_od_yloc(lch_unit, h, None, None, None, None,
                                            od_min_density=density_targ)
        if not od_y_list:
            return
        ...
        od_lay = mos_layer_table['OD_dummy']
        for idx, (od_yb, od_yt) in enumerate(od_y_list):
            ...
            for od_xl, od_xr in od_x_list:
                ...
                self.draw_od(template, od_lay, box)
                po_xl = od_xl + po_od_extx
                po_xr = po_xl + lch_unit
                nx = 1 + ((od_xr - po_xr - po_od_extx) // sd_pitch)
    ...
    def _draw_vertical_vias(
        ...
        for bot_lay_id in range(start_layer, top_layer):
            ...
            if bot_lay_id > 0:
                template.add_rect(
                    lay_name_table[bot_lay_id],
                    BBox(xlb, mbot_yb, xrb, mbot_yt, res, unit_mode=True),
                    nx=num,
                    spx=pitch,
                    unit_mode=True)
            if bot_lay_id == top_layer - 1:
                template.add_rect(
                    lay_name_table[bot_lay_id + 1],
                    BBox(xlt, mx_yb, xrt, mx_yt, res, unit_mode=True),
                    nx=num,
                    spx=pitch,
                    unit_mode=True)
            via_drawn = self._add_via_with_arr_constraint(
                template, mbot_yb, mbot_yt, mx_yb, mx_yt, via_type, x0, via_w, via_h, num, pitch,
                via_sp, via_benc_le, via_tenc_le, arr_nmax, arr_sp, via_enc1, via_enc2)
            if via_drawn:
                if m1_yb is not None and start_layer == 0:
                    ...
                    template.add_rect(
                        m1_name,
                        BBox(m1_xl, m1_yb, m1_xr, mx_yt, res, unit_mode=True),
                        nx=num,
                        spx=pitch,
                        unit_mode=True)
